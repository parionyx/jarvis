...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    async def _poll_messages(self) -> None:
        ...
        # Write sentinel so legacy inbound_reply_daemon knows not to start.
        _sentinel_path = self._session_path / ".adapter_polling_active"
        try:
        ...
        try:
                    ...
                    async with self._http_session.get(
                        ...
                        if resp.status == 200:
                            ...
                            for msg_data in messages:
                                ...
                                if event:
                                    # Fire-and-forget: mark read and acknowledge bridge receipt
                                    asyncio.create_task(self._send_read_receipt(msg_data))
                                    asyncio.create_task(self._send_ack(msg_data))
            ...
            # Remove sentinel so daemon knows adapter is no longer polling.
            # Avoid unlinking if a new task has already taken over (e.g. during rapid reconnect).
            try:
                if getattr(self, "_poll_task", None) is asyncio.current_task():
                    _sentinel_path.unlink(missing_ok=True)
            except OSError:
                pass

    async def _send_ack(self, data: Dict[str, Any]) -> None:
        """Acknowledge message processing to the bridge so it is removed from the inbox."""
        if not self._http_session:
            return
        msg_id = data.get("id") or data.get("messageId") or (data.get("key") or {}).get("id")
        if not msg_id:
            return
        try:
            import aiohttp
            ...
            async with self._http_session.post(
                f"http://127.0.0.1:{self._bridge_port}/messages/ack",
                json={"messageId": msg_id},
                timeout=aiohttp.ClientTimeout(total=5),
            ) as resp:
                if resp.status != 200:
                    logger.debug(
                        "[%s] WhatsApp message ack returned HTTP %s",
                        self.name,
                        resp.status,
                    )
        except Exception as exc:
            logger.debug("[%s] WhatsApp message ack failed: %s", self.name, exc)
