...
def cmd_dashboard(args):
    ...
    #
    # Threaded off the READY path (boot audit): the manifest walk sits
    # serially before the web_server import and delayed the backend ready
    # announce by its full cold-start cost. The sync is idempotent and
    # consumers read skills per-request, so a few seconds of lag at boot is
    # invisible; failures stay swallowed inside the helper.
    import threading as _skills_sync_threading

    _skills_sync_threading.Thread(
        target=_sync_bundled_skills_quietly, name="bundled-skills-sync", daemon=True
    ).start()
