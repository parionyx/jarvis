...
def _start_early_gateway_warm() -> None:
    """Start warming at MODULE IMPORT time for desktop-spawned backends.

    The lifespan used to pay the full warm serially before announcing
    readiness (~15-30s cold Windows, #60800) — measured as the dominant
    23s slice of a ~31s backend wait. Importing THIS module happens right
    after skills-sync/plugin discovery kick off, so running the warm in a
    daemon thread here overlaps its cost with plugin discovery, the FastAPI
    app build and uvicorn startup instead of stacking after them. By the
    time the lifespan calls :func:`_warm_gateway_module` again, every chain
    is already in sys.modules and the second call is a near-no-op.

    Gated on HERMES_DESKTOP=1 (the desktop spawn pins it) so plain CLI /
    gateway processes keep their existing behaviour. Imports hold the GIL,
    but at module-import time there is no running event loop to stall —
    the GH-73083 constraint does not apply on this path.
    """
    global _warm_thread
    if os.getenv("HERMES_DESKTOP") != "1" or _warm_thread is not None:
        return
    _warm_thread = threading.Thread(
        target=_warm_gateway_module, name="gateway-warm", daemon=True
    )
    _warm_thread.start()


_start_early_gateway_warm()
...
async def _lifespan(app: "FastAPI"):
    ...
    #
    # Boot-audit update: _start_early_gateway_warm() kicks the SAME warm off
    # at module-import time for desktop spawns. Re-running it here while that
    # thread is mid-import would BLOCK on the per-module import locks until
    # it finishes — which is exactly the 20s+ pre-READY stall this exists to
    # avoid. So: never double-block. If the early warm is still running, let
    # the socket open NOW; the remaining chains land in sys.modules moments
    # later, and first-hit RPC handlers already run their imports off the
    # loop thread (resolve_skin via to_thread in tui_gateway/ws.py).
    if _warm_thread is not None and _warm_thread.is_alive():
        _log.info("gateway warm already running in background; opening server without waiting")
    else:
...
_BUILTIN_DASHBOARD_THEMES = [
    ...
    {"name": "jarvis",    "label": "J.A.R.V.I.S.",   "description": "Cinematic AI operating system — Iron Man inspired"},
]
