...
class ProfileRoute:
    ...
    def matches(
        ...
        if self.thread_id and self.thread_id != thread_id:
            return False
        if self.chat_id:
            matched = (self.chat_id == chat_id or self.chat_id == parent_chat_id)
            if not matched and self.platform == "whatsapp":
                try:
                    from gateway.whatsapp_identity import (
                        canonical_whatsapp_identifier,
                        expand_whatsapp_aliases,
                    )
                    route_canonical = canonical_whatsapp_identifier(self.chat_id)
                    chat_canonical = canonical_whatsapp_identifier(chat_id or "")
                    if route_canonical and route_canonical == chat_canonical:
                        matched = True
                    elif bool(expand_whatsapp_aliases(self.chat_id) & expand_whatsapp_aliases(chat_id or "")):
                        matched = True
                except Exception:
                    pass
            if not matched:
