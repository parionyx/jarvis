...
import { abortCurrentComputerAction, executeComputerAction } from './computer-executor'
import { startJarvisAgentSupervisor, stopJarvisAgent } from './jarvis-agent-supervisor'
...
// Fix LiveKit WebRTC ICE server DNS resolution on Windows (-105 ERR_NAME_NOT_RESOLVED)
...
// ── Real-Time System Telemetry (Task Manager matching) ───────────────────────
let prevCpuTimes = os.cpus()
let prevNetBytes = { rx: 0, tx: 0, time: Date.now() }

function getRealNetworkSpeed() {
  try {
    const output = execFileSync('netstat', ['-e'], { encoding: 'utf8', timeout: 600, windowsHide: true })
    const lines = output.split(/\r?\n/)
    for (const line of lines) {
      if (line.toLowerCase().trim().startsWith('bytes')) {
        const parts = line.trim().split(/\s+/).slice(1)
        if (parts.length >= 2) {
          const rx = parseInt(parts[0], 10) || 0
          const tx = parseInt(parts[1], 10) || 0
          const now = Date.now()
          const dt = Math.max(0.2, (now - prevNetBytes.time) / 1000)

          let downMbps = 0
          let upMbps = 0
          if (prevNetBytes.rx > 0 && rx >= prevNetBytes.rx) {
            downMbps = Math.round(((rx - prevNetBytes.rx) * 8 / (1024 * 1024 * dt)) * 10) / 10
            upMbps = Math.round(((tx - prevNetBytes.tx) * 8 / (1024 * 1024 * dt)) * 10) / 10
          }
          prevNetBytes = { rx, tx, time: now }
          return { downMbps, upMbps, connected: true }
        }
      }
    }
  } catch {}
  return { downMbps: 0, upMbps: 0, connected: true }
}

// ── J.A.R.V.I.S. LiveKit token minting ──────────────────────────────────────
// The API secret NEVER ships in the renderer bundle: the main process reads it
// from the environment (LIVEKIT_* vars) or the voice agent's .env, mints a
// short-lived HS256 access token here, and hands the renderer only the token.
const LIVEKIT_ENV_CANDIDATES = [
  process.env.JARVIS_LIVEKIT_ENV,
  path.join(app.getPath('documents'), 'Jarvis', 'livekit_agent', '.env')
].filter(Boolean) as string[]

function readLiveKitCredentials(): { url: string; apiKey: string; apiSecret: string } | null {
  const url = process.env.LIVEKIT_URL || 'wss://jarvis-mnse77fg.livekit.cloud'
  ...
  for (const envPath of LIVEKIT_ENV_CANDIDATES) {
    try {
      ...
      const resolved = {
        url: pick('LIVEKIT_URL') || url,
        apiKey: pick('LIVEKIT_API_KEY'),
        apiSecret: pick('LIVEKIT_API_SECRET')
      }
      if (resolved.apiKey && resolved.apiSecret) {
        return resolved as { url: string; apiKey: string; apiSecret: string }
      }
    } catch {
      // try next candidate
    }
...
ipcMain.handle('hermes:livekit:token', async () => {
  const creds = readLiveKitCredentials()

  if (!creds) {
    return { url: null, token: null, error: 'LiveKit credentials not found (LIVEKIT_API_KEY/SECRET)' }
  }
  ...
  const payload = base64Url(
    JSON.stringify({
      iss: creds.apiKey,
      sub: `Tony-Stark-${Math.random().toString(36).slice(2, 7)}`,
      name: 'Tony Stark',
      nbf: now - 5,
      exp: now + 3600 * 6,
      video: {
        room: 'jarvis-control-room',
        roomJoin: true,
        canPublish: true,
        canSubscribe: true,
        canPublishData: true
  ...
  const signature = crypto.createHmac('sha256', creds.apiSecret).update(`${header}.${payload}`).digest()
  const token = `${header}.${payload}.${base64Url(signature)}`

  return { url: creds.url, token }
})

ipcMain.handle('hermes:system:telemetry', async () => {
  // 1. CPU Usage
  ...
  const cpuPercent = totalDiff > 0 ? Math.max(1, Math.min(100, Math.round(((totalDiff - idleDiff) / totalDiff) * 1000) / 10)) : 0
  const cpuModel = currentCpus[0]?.model || 'Processor'
  const cpuCores = currentCpus.length
  const cpuSpeedMhz = currentCpus[0]?.speed || 0
  ...
  // 4. Network Speed & Active Interfaces
  const netSpeed = getRealNetworkSpeed()
  const ifaces = os.networkInterfaces()
  let activeAdapterName = 'Active Adapter'
  for (const [name, addrs] of Object.entries(ifaces)) {
    if (addrs?.some(a => !a.internal && a.family === 'IPv4')) {
  ...
  const isBattery = onBatteryPower === true
  ...
  return {
    ...
    network: {
      downMbps: netSpeed.downMbps,
      upMbps: netSpeed.upMbps,
      connected: netSpeed.connected,
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to J.A.R.V.I.S. Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
ipcMain.handle('hermes:notify', (_event, payload) => {
  ...
  const notification = new Notification({
    title: payload?.title || 'J.A.R.V.I.S.',
    body: payload?.body || '',
...
ipcMain.handle('hermes:screen:capture', async (_event, options: { screenIndex?: number } = {}) => {
  const displays = screen.getAllDisplays()
  const primaryDisplay = screen.getPrimaryDisplay()
  const targetDisplay =
    options?.screenIndex !== undefined && displays[options.screenIndex]
      ? displays[options.screenIndex]
      : primaryDisplay

  const { width, height } = targetDisplay.bounds
  const scale = targetDisplay.scaleFactor || 1

  const maxW = Math.min(Math.round(width * scale), 1920)
  const maxH = Math.min(Math.round(height * scale), 1080)
  ...
  const filePath = await writeComposerImage(pngBuffer, '.png')
  const dataUrl = source.thumbnail.toDataURL()

  let activeWindow: { appName?: string; bounds?: unknown; windowTitle?: string } | null = null
  try {
    const titlesAvailable = IS_MAC ? systemPreferences.getMediaAccessStatus?.('screen') === 'granted' : true
    const read = await readWindowBelow(process.pid, { height: 100, width: 100, x: 0, y: 0 }, titlesAvailable)
    if (read && 'window' in read && read.window) {
      activeWindow = {
        appName: read.window.app || read.frontmost?.app,
        bounds: read.window.bounds,
        windowTitle: read.window.title || read.frontmost?.title
      }
    }
  } catch {}
...
// JARVIS Phase 3: Computer Control / ACT Native Execution
...
app.whenReady().then(() => {
  ...
  // J.A.R.V.I.S. voice agent: warm the LiveKit worker alongside the app so
  // real-time voice is instant. No-op (idle) when the agent dir/venv is absent.
  try {
    startJarvisAgentSupervisor(path.join(resolveHermesHome(), 'logs'))
  } catch (error) {
    rememberLog(`[jarvis-agent] supervisor start failed: ${error instanceof Error ? error.message : String(error)}`)
  }
})
...
app.on('before-quit', event => {
  ...
  // Kill the J.A.R.V.I.S. voice agent worker first so no orphaned python
