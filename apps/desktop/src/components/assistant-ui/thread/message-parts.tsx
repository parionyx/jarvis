...
import { ButtonsTool, FormTool } from '@/components/assistant-ui/chat-ui-tool'
...
const ChainToolFallback: FC<TimelineToolCallProps> = props => {
  ...
  // Interactive chat widgets (desktop): persistent action chips + blocking
  // structured forms. Both render from the tool-call args; the form wires its
  // request_id from the parked `form.request` store entry.
  if (props.toolName === 'show_buttons') {
    return (
      <>
        <TimelineTimestamp className="mb-0.5 block" completedAt={props.completedAt} timestamp={props.timestamp} />
        <ButtonsTool {...props} />
