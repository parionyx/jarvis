...
import { GitForkIcon, RefreshCwIcon, SmilePlusIcon, XIcon } from '@/lib/icons'
import { extractPreviewTargets } from '@/lib/preview-targets'
import { useEnterAnimation } from '@/lib/use-enter-animation'

// Stable empty identity for the settled-parts selector â€” a fresh [] per render
// would re-derive the changed-files card on every message re-render.
const EMPTY_PARTS: readonly unknown[] = []

interface MessageActionProps {
  messageId: string
  /** Lazy accessor â€” reads the live message text at action time. Passing the
   *  text itself as a prop forces the whole footer to re-render on every
   *  streaming delta flush (the text changes ~30Ã—/s), which profiling showed
...
export const AssistantMessage: FC<{
  ...
  // the human conversation â€” collapse it under a compact notice ("Reply to
  ...
  // TurnActivityIndicator leaf â€” not the footer/preview/root subtree.
  ...
  // Whole-turn wall-clock seconds (set once at completion â€” referentially
  ...
  // event, a steer race) must never show one â€” a spinner above a later user
  ...
  // Preview targets only materialize once the turn completes â€” while running
  ...
  // retires it â€” the working tree it describes is already history by then.
  ...
  // parity â€” the transcript shows the event; the text is one click away).
  ...
  return (
    ...
    >
      ...
      {/* Last thing in the turn â€” under the action bar, the way Cursor ends a
  ...
  return (
    <div className="relative flex w-full shrink-0 items-center justify-end gap-1.5">
      ...
      {/* ONE slot, Slack-style: the picker trigger and the landed reaction are
          the same element, so reacting never shifts layout. Empty â†’ â˜º, hidden
          until hover like its action-bar neighbors (state lives in styles.css
          â€” the aui_msg-reactions rules outweigh Tailwind utilities here).
          Reacted â†’ the emoji itself, always visible at full strength, and
