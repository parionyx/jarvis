...
interface ToolsetConfigPanelProps {
   ...
   *  app-wide active one. Omitted (every other caller) â†’ app-wide active
 ...
 * server-computed `status` (keys âˆ§ Nous entitlement âˆ§ post-setup install
 * state). Older backends don't send `status` â€” fall back to the legacy
...
function providerStatus(provider: ToolProvider, envState: Record<string, boolean>): ToolProviderStatus {
  if (provider.status) {
    // Env-var edits patch envState locally without a refetch â€” a stale
...
function EnvVarField({ envVar, isSet, onSaved, onCleared, profile }: EnvVarFieldProps) {
  ...
  // Internal route change to Settings â†’ API Keys (tools sub-view) with the
...
interface PostSetupRunnerProps {
   ...
   *  (provider status === 'ready') â€” renders the resting "Installed" state
 ...
 * log inline â€” the GUI equivalent of the install step `hermes tools` runs
...
function PostSetupRunner({ toolset, postSetupKey, installed = false, onComplete, profile }: PostSetupRunnerProps) {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      // that will never exist â€” surface the failure and stop.
      if (!started.ok) {
...
interface ModelCatalogPickerProps {
  ...
  providerName: string
  /** True when this provider is the one written to config â€” selecting a model
...
/**
 * Backend model catalog â€” the GUI counterpart of the model picker `hermes
  ...
  useEffect(() => {
    ...
    getToolsetModels(toolset, providerName, profile)
      ...
      .catch(() => {
        ...
        // catalog â€” hide the section entirely rather than erroring.
        if (!cancelled) {
          setCatalog(null)
        }
      })
...
export function ToolsetConfigPanel({ toolset, onConfiguredChange, profile }: ToolsetConfigPanelProps) {
  ...
  // Which provider row is EXPANDED in the panel (purely presentational â€”
  ...
  // machinery onboarding uses: start â†’ open verification URL â†’ poll), then
  ...
  if (loading) {
    // Inline row, not a full block loader â€” a big centered spinner is what
  ...
  // Nothing to configure â†’ render nothing. An inspector explaining that there
  ...
  return (
    <div className="grid gap-2">
      {toolset === 'web' && cfg.active_search_backend !== undefined && (
        ...
        // (web.search_backend / web.extract_backend) â€” show which backend
        ...
        return (
          <div className="overflow-hidden rounded-xl bg-background/60" key={provider.name}>
            <button
              ...
              onClick={() => {
                // Row click only expands/collapses â€” activating a backend is
            ...
            {isExpanded && (
              <div className="grid gap-2 bg-muted/20 p-3">
                ...
                {(toolset !== 'web' || webCaps.length === 0) && (
                  // Explicit activation â€” the old row-click-selects UX gave no
