...
import { Box, Download, FileText, LayoutDashboard, PanelBottom, Terminal, Upload, Zap } from '@/lib/icons'
import { type KeybindContribution, KEYBINDS_AREA } from '@/lib/keybinds/actions'
import { TRANSCRIPT_DIRECTIVE_AREA, type TranscriptDirectiveContribution } from '@/lib/transcript-directives'
import { setYoloEnabled } from '@/lib/yolo-session'
import {
  $artifactStudioOpen,
  ARTIFACT_STUDIO_PANE_ID,
  closeArtifactStudio,
  initArtifactStudioEvents,
  openArtifactStudio
...
import { ArtifactStudioPane } from '../artifact-studio/artifact-studio-pane'
...
// ---------------------------------------------------------------------------
// Artifact Studio — the engineering workspace pane (JARVIS artifact_* tools).
// Summon-only like logs: registered while $artifactStudioOpen is on, revealed
// by `artifact.studio.open` (agent tool) or the palette row, and removed when
// dismissed so it never resurrects from persisted trees. The store's gateway
// event wiring is installed once here at boot.
// ---------------------------------------------------------------------------
let unregisterArtifactStudioPane: (() => void) | null = null
let artifactStudioEventsInstalled = false

const syncArtifactStudioPane = (open: boolean) => {
  if (!artifactStudioEventsInstalled) {
    artifactStudioEventsInstalled = true
    initArtifactStudioEvents()
  }

  if (open) {
    unregisterArtifactStudioPane ??= registry.register({
      id: ARTIFACT_STUDIO_PANE_ID,
      area: 'panes',
      title: 'artifacts',
      data: {
        placement: 'main',
      ...
      render: () => idle(<ArtifactStudioPane />)
    })
    revealTreePane(ARTIFACT_STUDIO_PANE_ID)
  } else {
    unregisterArtifactStudioPane?.()
    unregisterArtifactStudioPane = null

    const tree = $layoutTree.get()

    if (tree && allPaneIds(tree).includes(ARTIFACT_STUDIO_PANE_ID)) {
      removeTreePane(ARTIFACT_STUDIO_PANE_ID)
    }
  }
}

markCollapsePane(ARTIFACT_STUDIO_PANE_ID)
registerPaneCloser(ARTIFACT_STUDIO_PANE_ID, () => void closeArtifactStudio())
registerPaneOpener(ARTIFACT_STUDIO_PANE_ID, () => void openArtifactStudio())
syncArtifactStudioPane($artifactStudioOpen.get())
$artifactStudioOpen.listen(syncArtifactStudioPane)

registry.register(
  paletteToggle({
    id: 'artifacts.toggle',
    label: 'Toggle Artifact Studio',
    icon: Box,
    keywords: ['artifact', 'studio', 'engineering', 'cad', 'drone', 'jarvis'],
    // On-screen truth, not the store boolean — same reasoning as logs above.
    get: () => isPaneVisible(ARTIFACT_STUDIO_PANE_ID),
    set: () => togglePaneVisible(ARTIFACT_STUDIO_PANE_ID)
  })
)
