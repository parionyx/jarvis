...
import { useArtifactCommandBridge } from './hooks/use-artifact-command-bridge'
import {
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  // The Artifact Studio command bar's bridge: contextual engineering commands
  // queue in the studio and ride the same submitText pipeline here.
  useArtifactCommandBridge({ submitText })
  ...
  // Wake-word AUTO-ARM removed (user preference: no hands-free listener).
  // It used to call armWakeWord on every gateway open, opening the mic and
  // logging "Failed to open the wake-word microphone" on machines without
  // one. Manual voice entry points still work; config.yaml
  // `wake_word.enabled: false` keeps the backend side dormant too.
