...
import { captureScreenForVision } from '@/lib/vision-capture'
import { isVisionIntent } from '@/lib/vision-intent'
import {
  $composerAttachments,
...
export function useSubmitPrompt(deps: SubmitPromptDeps) {
  ...
  return useCallback(
    async (rawText: string, options?: SubmitTextOptions) => {
      ...
      let hasImage = attachments.some(a => a.kind === 'image')

      // JARVIS Phase 2 Screen Vision for typed messages:
      if (!hasImage && isVisionIntent(visibleText)) {
        const capture = await captureScreenForVision()

        if (capture.allowed && capture.attachment) {
          attachments.push(capture.attachment)
          hasImage = true

          if (capture.contextHeader) {
            visibleText = `${capture.contextHeader}\n${visibleText}`
          }
        } else if (!capture.allowed && capture.error) {
          notify({ kind: 'warning', message: capture.error, title: 'Vision Disabled' })
        }
      }
