...
import { type ChatFormRequest, normalizeFormFields, setChatFormRequest } from '@/store/chat-form'
...
export function handleInputRequestEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === 'form.request') {
    // show_form tool (desktop GUI): the Python side blocks on form.respond,
    // same lifecycle as clarify. Park per-session, upsert a stable tool row
    // (the inline card mounts from it even when tool.start was missed), and
    // flag needs-input so a background session's form is still discoverable.
    const requestId = typeof payload?.request_id === 'string' ? payload.request_id : ''
    const title = typeof payload?.title === 'string' ? payload.title.trim() : ''
    const fields = normalizeFormFields(payload?.fields)

    const submitLabel =
      typeof payload?.submit_label === 'string' && payload.submit_label.trim()
        ? payload.submit_label.trim()
        : 'Submit'

    if (requestId && title && fields.length > 0) {
      const request: ChatFormRequest = {
        fields,
        requestId,
        sessionId: sessionId ?? null,
        submitLabel,
      ...
      setChatFormRequest(request)

      if (sessionId) {
        upsertToolCall(
          sessionId,
          { args: { fields, submit_label: submitLabel, title }, name: 'show_form', tool_id: requestId },
          'running',
          event.type,
          occurredAt
        )
