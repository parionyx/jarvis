...
export type ArtifactKind = 'code' | 'html' | 'react' | 'svg'
...
// JSX/TSX fences that define at least one component graduate into live React
// artifacts. A fence that is only fragments of markup (no component shape)
// stays an ordinary code artifact.
const REACT_LANGUAGES = new Set(['jsx', 'tsx'])
const REACT_MIN_CHARS = 120

const REACT_COMPONENT_RE =
  /(?:export\s+default\s+)?(?:function\s+[A-Z]\w*\b|(?:const|let|var)\s+[A-Z]\w*[^;\n]{0,120}=>|class\s+[A-Z]\w*\s+extends\s+(?:React\.)?Component)/

const REACT_JSX_TAG_RE = /<[A-Za-z][\w.]*[\s/>]/
...
export function detectArtifact(language: string | undefined, code: string | undefined): ArtifactDetection | null {
  ...
  if (REACT_LANGUAGES.has(clean)) {
    // A component-shaped jsx/tsx fence with real JSX markup renders live.
    // Anything shorter or without both signals is plain code — a fragment of
    // JSX in prose shouldn't open a React runtime.
    if (trimmed.length >= REACT_MIN_CHARS && REACT_COMPONENT_RE.test(trimmed) && REACT_JSX_TAG_RE.test(trimmed)) {
      return {
        kind: 'react',
        language: clean,
        title: codeTitle(clean, trimmed)
      }
    }
      ...
      : kind === 'svg'
        ...
        : kind === 'react'
          ? language === 'tsx'
            ? '.tsx'
            : '.jsx'
          : DOWNLOAD_EXTENSION_BY_LANGUAGE[language] || '.txt'
