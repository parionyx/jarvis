...
describe('detectArtifact', () => {
  ...
  it('promotes a component-shaped jsx fence as a react artifact', () => {
    const jsx = `export default function Counter() {
  ...
  return (
    ...
    const detection = detectArtifact('jsx', jsx)

    expect(detection?.kind).toBe('react')
    expect(detection?.title).toBe('Counter')
  })

  it('promotes a typed tsx component (arrow, named export)', () => {
    const tsx = `export const Card: React.FC<{ title: string }> = ({ title }) => {
  return (
    <article>
      <h2>{title}</h2>
      <p>Body text that pads the length past the minimum artifact size.</p>
    </article>
  )
}`
    const detection = detectArtifact('tsx', tsx)

    expect(detection?.kind).toBe('react')
  })

  it('keeps short or component-less jsx fences as plain code', () => {
    expect(detectArtifact('jsx', '<div className="chip">hi</div>')).not.toBeNull()
    expect(detectArtifact('jsx', '<div className="chip">hi</div>')?.kind).toBe('code')

    const noJsx = `function helper(value) { return value * 2 }`.repeat(6)
    expect(detectArtifact('jsx', noJsx)?.kind).toBe('code')
  })
})
