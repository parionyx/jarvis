...
const ALLOWED_USERS = parseAllowedUsers(getArg('allowed-users', process.env.WHATSAPP_ALLOWED_USERS || '*'));
...
// Acknowledgement-based message inbox
const INBOX_LIMIT = 500;
const CLAIM_TIMEOUT_MS = 30000; // 30-second in-flight claim lease
const messageInbox = new Map(); // messageId -> { event, status: 'pending'|'in_flight'|'acked', receivedAt, claimedAt, attempts, lastError }
const ackedMessageIds = new Set();
const messageQueue = [];
const MAX_QUEUE_SIZE = 100;

// Outbound Idempotency Tracker
const OUTBOUND_IDEMPOTENCY_LIMIT = 2000;
const outboundIdempotency = new Map(); // idempotencyKey -> { status: 'in_progress' | 'sent' | 'failed', response: object, promise: Promise, createdAt: number }
...
async function startSocket() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Add to acknowledgement inbox
      const msgId = event.messageId || `${event.senderId}_${event.timestamp || Date.now()}`;
      if (!messageInbox.has(msgId) && !ackedMessageIds.has(msgId)) {
        messageInbox.set(msgId, {
          event,
          status: 'pending',
          receivedAt: Date.now(),
          claimedAt: null,
        ...
        if (messageInbox.size > INBOX_LIMIT) {
          const oldestKey = messageInbox.keys().next().value;
          messageInbox.delete(oldestKey);
        }
      }
...
// Fetch pending incoming messages (acknowledgement-based with lease claim)
app.get('/messages', (req, res) => {
  const now = Date.now();
  const shouldClaim = req.query.claim !== 'false' && req.query.peek !== 'true';
  const deliverable = [];

  for (const [msgId, entry] of messageInbox.entries()) {
    if (entry.status === 'acked') continue;

    // Available if pending, or in_flight and lease expired
    if (entry.status === 'pending' || (entry.status === 'in_flight' && now - (entry.claimedAt || 0) > CLAIM_TIMEOUT_MS)) {
      if (shouldClaim) {
        entry.status = 'in_flight';
        entry.claimedAt = now;
        entry.attempts += 1;
      }
      deliverable.push(entry.event);
    }
  }
...
// Acknowledge processed message (removes from active inbox)
app.post('/messages/ack', (req, res) => {
  ...
  if (messageInbox.has(messageId)) {
    const entry = messageInbox.get(messageId);
    entry.status = 'acked';
    messageInbox.delete(messageId);
    ackedMessageIds.add(messageId);
    if (ackedMessageIds.size > 2000) {
      const oldest = ackedMessageIds.values().next().value;
      ackedMessageIds.delete(oldest);
    }
    return res.json({ success: true, messageId, status: 'acked' });
  }

  if (ackedMessageIds.has(messageId)) {
    return res.json({ success: true, messageId, status: 'already_acked' });
  }

  return res.status(404).json({ error: 'Message not found in inbox', messageId });
});

// Negative acknowledgement (releases in-flight claim immediately)
app.post('/messages/nack', (req, res) => {
  const messageId = req.body.messageId || req.body.message_id || req.body.id;
  const error = req.body.error || 'processing_failed';
  if (!messageId) {
    return res.status(400).json({ error: 'messageId is required' });
  }
  ...
  if (messageInbox.has(messageId)) {
    ...
    return res.json({ success: true, messageId, status: 'pending' });
  }

  return res.status(404).json({ error: 'Message not found in inbox', messageId });
});

// Inject inbound message into inbox (for testing and simulation)
app.post('/messages/inject', (req, res) => {
  const { messageId, chatId, senderId, body, text, message } = req.body;
  const msgText = body || text || message || '';
  const msgId = messageId || `TEST_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
  const event = {
    messageId: msgId,
    chatId: chatId || '919999888777@s.whatsapp.net',
    senderId: senderId || chatId || '919999888777@s.whatsapp.net',
    body: msgText,
    timestamp: Math.floor(Date.now() / 1000),
    isGroup: false,
    hasMedia: false,
  ...
  res.json({ success: true, messageId: msgId, event });
});

// Send a message
app.post('/send', async (req, res) => {
  const { chatId, message, replyTo, idempotencyKey, idempotency_key } = req.body;
  const key = idempotencyKey || idempotency_key || req.headers['x-idempotency-key'];

  if (key) {
    const existing = outboundIdempotency.get(key);
    if (existing) {
  ...
  if (key) {
    outboundIdempotency.set(key, {
      status: 'in_progress',
      promise: inFlightPromise,
      createdAt: Date.now()
    });
    if (outboundIdempotency.size > OUTBOUND_IDEMPOTENCY_LIMIT) {
      const oldestKey = outboundIdempotency.keys().next().value;
      outboundIdempotency.delete(oldestKey);
    }
  }
...
app.post('/send-media', async (req, res) => {
  const { chatId, filePath, mediaType, caption, fileName, idempotencyKey, idempotency_key } = req.body;
  const key = idempotencyKey || idempotency_key || req.headers['x-idempotency-key'];
  ...
  try {
    if (!existsSync(filePath)) {
      if (key) outboundIdempotency.delete(key);
    ...
    trackSentMessageId(sent);
    messageStore.remember(sent);
    const responseData = { success: true, messageId: sent?.key?.id };
...
app.get('/health', (req, res) => {
  res.json({
    ...
    queueLength: messageQueue.length,
    inboxSize: messageInbox.size,
    inbox: Array.from(messageInbox.entries()),
