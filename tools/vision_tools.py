...
def capture_screen_tool() -> Dict[str, Any]:
    """Capture the primary display screen using PIL ImageGrab on Windows/Desktop."""
    try:
        from PIL import ImageGrab
        hermes_dir = get_hermes_dir()
        screenshot_dir = hermes_dir / "cache" / "screenshots"
        screenshot_dir.mkdir(parents=True, exist_ok=True)
        stamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"screen_{stamp}_{uuid.uuid4().hex[:6]}.png"
