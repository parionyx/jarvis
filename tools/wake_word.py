"""Wake-word ("Hey Jarvis") detection — hands-free session trigger.
...
* **openwakeword** (default, free, no API key) — loads an ONNX model. Defaults
  to the bundled "hey jarvis" model (``tools/wakewords/``) so the wake word
...
# Minimum gap between two consecutive wake fires, so one "hey jarvis" can't
# retrigger across several frames while the caller is still reacting.
_FIRE_COOLDOWN_SECONDS = 1.5
_START_TIMEOUT_SECONDS = 5.0

# Ambient-speech rejection: openWakeWord scores one ~80ms frame at a time.
# Requiring 2 consecutive over-threshold frames (160ms) rejects single-frame clicks
# while remaining highly responsive to natural accented pronunciations.
...
# Bundled "hey jarvis" model (tools/wakewords/) — the default, so the wake word
# works out of the box. Config names in _ALIASES resolve to it, not a built-in.
_BUNDLED_MODEL_NAME = "hey_jarvis"
...
def _bundled_wakeword_path(framework: str = "onnx", model_name: str = _BUNDLED_MODEL_NAME) -> str:
    """Path to the shipped wakeword model (.onnx/.tflite) for ``framework``."""
    ext = "tflite" if str(framework).strip().lower() == "tflite" else "onnx"
    path = os.path.join(os.path.dirname(__file__), "wakewords", f"{model_name}.{ext}")
    if os.path.exists(path):
        return path
    fallback = os.path.join(os.path.dirname(__file__), "wakewords", f"{model_name}.onnx")
    if os.path.exists(fallback):
        return fallback
    return os.path.join(os.path.dirname(__file__), "wakewords", f"hey_hermes.{ext}")
...
class _SherpaKwsEngine(_Engine):
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        phrase = str(_get(cfg, "phrase") or "hey jarvis").strip()
