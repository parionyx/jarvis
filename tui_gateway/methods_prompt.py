...
@method("form.respond")
def _(rid, params: dict) -> dict:
    # show_form tool (desktop GUI): `answer` is a JSON string of {field: value}.
    # allow_expired=True for the same lifecycle reason as clarify — the form
    # card can outlive a server-side timeout after a reconnect drops
    # tool.complete; a late submit must resolve gracefully.
    return _respond(rid, params, "answer", allow_expired=True)
