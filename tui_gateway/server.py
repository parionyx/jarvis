...
def _tool_lifecycle_required_for_ui(name: str) -> bool:
    ...
    # show_buttons/show_form render their widgets from the tool-call part too.
    return name in ("clarify", "setup_mcp", "show_buttons", "show_form")
...
def _agent_cbs(sid: str) -> dict:
    callbacks = {
        ...
        # show_form tool (desktop GUI): blocking structured-input card. Same
        # lifecycle as clarify — the renderer renders the inline form from the
        # tool-call part, wires request_id from form.request, and answers
        # form.respond with a JSON string of {field: value}.
        "form_callback": lambda title, fields, submit_label="Submit": _block(
            "form.request",
            sid,
            {"title": title, "fields": fields, "submit_label": submit_label},
            timeout=_clarify_timeout_seconds(),
        ),
