...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # HUNTER V2: Suppress for lead-hunter profile to prevent architecture disclosure.
    _suppress_help = False
    try:
        _h = _agent_home(agent)
        if _h is not None and _profile_name_for_home(_h) == "lead-hunter":
            _suppress_help = True
        elif _h is None:
            from agent.file_safety import _resolve_active_profile_name
            if _resolve_active_profile_name() == "lead-hunter":
                _suppress_help = True
    except Exception:
        pass
    
    if not _suppress_help:
