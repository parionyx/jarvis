...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if function_name == "todo":
        ...
        elif function_name == "show_form":
            def _execute(next_args: dict) -> Any:
                from tools.chat_ui_tool import show_form_tool as _show_form_tool
                return _show_form_tool(
                    title=next_args.get("title", ""),
                    fields=next_args.get("fields"),
                    submit_label=next_args.get("submit_label", ""),
                    callback=getattr(agent, "form_callback", None),
                )
            function_result, function_args, middleware_trace, _execution_blocked, _execution_dispatched = _managed_values(_run_agent_tool_execution_middleware(
                agent,
                function_name=function_name,
            ...
            tool_duration = time.time() - tool_start_time
            if agent._should_emit_quiet_tool_messages():
                agent._vprint(f"  {_get_cute_tool_message_impl('show_form', function_args, tool_duration, result=function_result)}")
